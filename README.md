...
A high-performance, cinematic personal portfolio built with Next.js 16, GSAP, Three.js, and CSS Modules. Every piece of personal content lives in a single data file, so the whole site can be re-skinned in one place.

Built and maintained by Kavya Narayanappa.
...
- [ ] Updated every field in `data/profile.json`
- [ ] Set `SITE_URL` in `lib/siteConfig.js`
- [ ] Replaced the photo/video assets in `public/assets/`
- [ ] Swapped the favicons in `public/favicons/`
- [ ] Adjusted the color tokens in `app/globals.css` (optional)

## License

MIT. Free to clone, adapt, and use as your own portfolio.
